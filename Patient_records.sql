...
SELECT DISEASE,COUNT(PATIENTID) AS COUNT
FROM PATIENTRECORDS
GROUP BY DISEASE;
...
SELECT PATIENTNAME,AGE,GENDER,CITY
FROM PATIENTRECORDS
WHERE PATIENTNAME
LIKE 'A%';
...
SELECT PATIENTNAME,LENGTH(PATIENTNAME) AS LENGTH
FROM PATIENTRECORDS;

#Q14 Display uppercase names
SELECT UPPER(PATIENTNAME) AS NAME
FROM PATIENTRECORDS;
...
CASE
WHEN AGE BETWEEN 18 AND 30 THEN "YOUNG AGE"
...
RANK() OVER(PARTITION BY DISEASE ORDER BY TREATMENTCOST DESC) AS TOP_RANK
FROM PATIENTRECORDS;
...
SELECT 
    ROUND((COUNT(*) * 100.0 / (SELECT COUNT(*) FROM PATIENTRECORDS))) AS Male_Percentage
FROM PATIENTRECORDS
WHERE Gender = 'Male';
...
SELECT CITY,COUNT(PATIENTID) AS HIGHEST_PATIENTS
FROM PATIENTRECORDS
GROUP BY CITY
...
SELECT CITY,COUNT(PATIENTID) AS LOWEST_PATIENTS
FROM PATIENTRECORDS
GROUP BY CITY
ORDER BY LOWEST_PATIENTS 
LIMIT 1;
...
SELECT DISEASE,COUNT(PATIENTID) AS HIGHEST_PATIENTS
FROM PATIENTRECORDS  
GROUP BY DISEASE 
ORDER BY HIGHEST_PATIENTS DESC
LIMIT 1;
...
FROM PATIENTRECORDS  
GROUP BY DISEASE 
ORDER BY HIGHEST_PATIENTS 
LIMIT 1;

#Q13. How many patients are diagnosed with Heart Disease?
SELECT DISEASE,COUNT(PATIENTID) AS PATIENTS
FROM PATIENTRECORDS
GROUP BY DISEASE
HAVING DISEASE = "Heart Disease";

#Q14. How many patients are diagnosed with Diabetes?
SELECT DISEASE,COUNT(PATIENTID) AS PATIENTS
FROM PATIENTRECORDS
GROUP BY DISEASE
HAVING DISEASE = "Diabetes";

#Q15. What percentage of patients have Hypertension?
SELECT 
(COUNT(*) * 100 / (SELECT COUNT(*) FROM PatientRecords)) AS Hypertension_Percentage
FROM PatientRecords
...
SELECT DISEASE,AVG(AGE) AS OLDEST_PATIENTS
FROM PATIENTRECORDS
GROUP BY DISEASE
ORDER BY OLDEST_PATIENTS DESC
LIMIT 1;
...
FROM PATIENTRECORDS
GROUP BY DISEASE
ORDER BY OLDEST_PATIENTS 
LIMIT 1;

#Q19. What is the disease distribution across cities?
SELECT CITY,DISEASE,COUNT(PATIENTID) AS PATIENTS
FROM PATIENTRECORDS
...
SELECT SUM(TREATMENTCOST) AS TOTAL_REVENUE_COST
FROM PATIENTRECORDS;

#Q22. What is the average treatment cost?
SELECT AVG(TREATMENTCOST) AS AVG_TREATMENT_COST
FROM PATIENTRECORDS;

#Q23. What is the maximum treatment cost?
SELECT MAX(TREATMENTCOST) AS MAX_TREATMENT_COST
FROM PATIENTRECORDS;

#Q24. What is the minimum treatment cost?
SELECT MIN(TREATMENTCOST) AS MIN_TREATMENT_COST
FROM PATIENTRECORDS;

#Q25. Which disease generates the highest revenue?
SELECT DISEASE,SUM(TREATMENTCOST) AS HIGHEST_REVENUE
FROM PATIENTRECORDS
GROUP BY DISEASE
...
SELECT DISEASE,SUM(TREATMENTCOST) AS LOWEST_REVENUE
FROM PATIENTRECORDS
GROUP BY DISEASE
...
SELECT DISEASE,avg(TREATMENTCOST) AS avg_REVENUE
FROM PATIENTRECORDS
GROUP BY DISEASE;
...
SELECT CITY,DISEASE,TREATMENTCOST
FROM PATIENTRECORDS
ORDER BY TREATMENTCOST DESC
LIMIT 1;
...
SELECT AVG(DATEDIFF(DischargeDate, AdmissionDate)) AS AVERAGE_STAY
FROM PATIENTRECORDS;

# Q32. What is the maximum hospital stay duration?
SELECT MAX(DATEDIFF(DischargeDate, AdmissionDate)) AS MAXIMUM_STAY
FROM PATIENTRECORDS;

#Q33. What is the minimum hospital stay duration?
SELECT MIN(DATEDIFF(DischargeDate, AdmissionDate)) AS MINIMUM_STAY
FROM PATIENTRECORDS;
...
SELECT DISEASE,
AVG(DATEDIFF(DischargeDate, AdmissionDate)) AS AVERAGE_STAY
FROM PATIENTRECORDS
GROUP BY DISEASE
...
SELECT ROUND(COUNT(*) * 100.0 /(SELECT COUNT(*) FROM PATIENTRECORDS), 2) AS PERCENTAGE
FROM PATIENTRECORDS
...
SELECT Disease,AVG(BloodPressure) AS Average_BP
FROM PATIENTRECORDS
GROUP BY Disease
...
SELECT Disease,AVG(Cholesterol) AS Average_Cholesterol
FROM PATIENTRECORDS
GROUP BY Disease
...
SELECT Disease,AVG(BMI) AS Average_BMI
FROM PATIENTRECORDS
GROUP BY Disease
...
SELECT SmokingStatus,COUNT(*) AS Total_Patients
FROM PATIENTRECORDS
